...
const MenuPage = () => {
  return (
    <div className='p-4 lg:px-20 xl:px-40 flex flex-col md:flex-row items-center h-[calc(100vh-6rem)] md:h-[calc(100vh-9rem)]'>
      {menu.map((category) => (
        <Link href={`/menu/${category.slug}`} key={category.id} className='w-full h-1/3 md:h-1/2 bg-cover p-8' style={{ backgroundImage: `url(${category.img})`}}>
          <div className={`text-${category.color} w-1/2`}>
            ...
            <p className="text-xs my-8">{category.desc}</p>
